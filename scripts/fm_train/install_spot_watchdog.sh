#!/usr/bin/env bash
# Install the systemd timer that polls Spot VMs and restarts preempted ones.
set -euo pipefail
...
[Unit]
Description=Poll Spot VMs every minute; restart only after preemption
