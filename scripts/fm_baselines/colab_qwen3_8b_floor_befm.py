#!/usr/bin/env python3
"""Qwen3-8B-Base full BehaviorBench floor via vLLM OpenAI server.

Serve with vLLM (batched), evaluate with BehaviorBench LocalModel concurrency.
...
def ensure_vllm() -> str:
    ...
    try:
        ...
        pass
    sh(f"{py} -m pip install -q -U 'vllm>=0.6.0'")
    return py


def install_eval(eval_py: str) -> None:
...
def task_already_done(task: str) -> bool:
    ...
    for p in out.rglob("*.json"):
        try:
            ...
            continue
        names = {t.get("task_name") for t in (d.get("tasks") or []) if isinstance(t, dict)}
...
def main() -> None:
    ...
    print("resume_done", done, flush=True)
