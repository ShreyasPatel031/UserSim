...
def build_retell_study():
    ...
    # NOTE: Insights should be generated from actual trace data using synthesize_insights.py
    # Do NOT add hardcoded insight strings here - they must come from LLM synthesis of traces
    retell_strengths_vs_others = []  # Will be populated by LLM synthesis
    retell_weaknesses_vs_others = []  # Will be populated by LLM synthesis
    ...
    study = {
        ...
        "summary": {
            ...
            "top_friction": unique_dislikes[:6],  # From actual trace data only
            "top_strengths": unique_likes[:6],  # From actual trace data only
            "segment_fit_score": round(10 * success_count / max(1, total)),
            "segment_fit_rationale": f"Based on {total} simulated user sessions across {len(personas)} personas.",
            "conversion_outlook": f"{success_count} of {total} users completed their tasks.",
            "recommendations": [],  # To be generated by LLM synthesis
            "insights_from_traces": True,  # Flag that insights come from actual data
        },
        "preference_summary": {
