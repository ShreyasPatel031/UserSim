const PLATFORMS = ["recurse", "langchain", "llamaindex"];
const PLAT_LABEL = { recurse: "Recurse", langchain: "LangChain", llamaindex: "LlamaIndex" };
...
const _activeIdx = { recurse: 0, langchain: 0, llamaindex: 0 };
...
function legend() {
  return `<div class="legend">
    <span class="recurse">Recurse</span>
    <span class="langchain">LangChain</span>
    <span class="llamaindex">LlamaIndex</span>
  </div>`;
}
  ...
  analyticsRoot.innerHTML = `
    ...
    <div class="analytics-grid">
      <div class="chart-card">
        ...
        ${legend()}
        ${barRows(prefShare)}
        <p class="sub" style="margin-top:.75rem;margin-bottom:0">Counts: Recurse ${prefs.recurse || 0} · LangChain ${prefs.langchain || 0} · LlamaIndex ${prefs.llamaindex || 0}</p>
      </div>
      <div class="chart-card">
...
async function loadAnalytics() {
  const res = await fetch("/api/recurse/analytics");
...
async function loadStudies() {
  const res = await fetch("/api/recurse/studies");
  const data = await res.json();
...
async function loadStudy(studyId) {
  const res = await fetch(`/api/recurse/studies/${encodeURIComponent(studyId)}`);
...
function renderGoal(goalKey) {
  ...
  const sample = byPlatform.recurse || byPlatform.langchain || byPlatform.llamaindex;
