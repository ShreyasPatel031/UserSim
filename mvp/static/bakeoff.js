        ...
        <div class="persona-body">
          ...
          <details class="persona-goals">
            ...
            <table class="persona-table" style="margin-top:.5rem">
              <thead><tr><th>Goal</th><th>Bland</th><th>Vapi</th><th>Retell</th><th>Pick</th></tr></thead>
  ...
  analyticsRoot.innerHTML = `
    ...
    <div class="analytics-grid">
      <div class="chart-card">
        ...
        ${legend()}
        ${barRows(prefShare)}
        <p class="sub" style="margin-top:.75rem;margin-bottom:0">Counts: Bland ${prefs.bland || 0} · Vapi ${prefs.vapi || 0} · Retell ${prefs.retell || 0}</p>
      </div>
      <div class="chart-card">
        <h3>Avg steps to done</h3>
        <p class="sub">Lower is better — actions among successful runs</p>
        ${legend()}
        ${PLATFORMS.map((p) => {
      ...
      <div class="chart-card">
        <h3>Task completion (ceiling)</h3>
        <p class="sub">Did the agent reach the goal page? Near 100% — not the discriminator</p>
        ${legend()}
...
async function loadAnalytics() {
  const res = await fetch("/api/bakeoff/analytics");
