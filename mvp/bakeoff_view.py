...
# Full results locally; packaged mvp/bakeoff_data on Vercel (results/ is .vercelignored).
_PACKAGED = Path(__file__).resolve().parent / "bakeoff_data"
_RESULTS = ROOT / "results" / "capability"
CAPABILITY_DIR = (
    _RESULTS
    if (_RESULTS / "product_persona_p1_browser_use_p1_all_v1.json").is_file()
    else _PACKAGED
)
...
# Trace columns default to all three; analytics always includes Vapi.
ACTIVE_PLATFORMS = frozenset({"bland", "vapi", "retell"})
TRACE_PLATFORMS = ("bland", "vapi", "retell")
...
def list_studies() -> list[dict]:
    ...
    for path in sorted(CAPABILITY_DIR.glob("product_persona_p*_browser_use_*_all_v1.json")):
        ...
        active_runs = [
            r for r in (data.get("runs") or []) if r.get("website") in ACTIVE_PLATFORMS
        ]
        out.append(
            {
                "id": path.stem,
                ...
                "n_runs": len(active_runs) or data.get("n") or len(data.get("runs") or []),
                "successes": sum(1 for r in active_runs if r.get("success")),
...
def load_study(study_id: str) -> dict:
    ...
    # Prefer prebaked payload (screenshot URLs already point at /bakeoff-traces/...).
    prebaked = Path(__file__).resolve().parent / "bakeoff_data" / "prebaked" / f"{study_id}.json"
    if prebaked.is_file():
        return json.loads(prebaked.read_text())

    comparative = {}
    if comp_path.is_file():
        comparative = json.loads(comp_path.read_text())

    runs = [r for r in runs if r.get("website") in ACTIVE_PLATFORMS]
    ...
    n_ok = sum(1 for r in agent_results if r.get("success"))
    return {
        "study_id": study_id,
        "status": "complete",
        "phase": "Complete",
        "headline": f"Persona bakeoff — {n_ok}/{len(agent_results)} tasks succeeded",
        "personas": personas,
        "tasks": tasks,
...
def parse_trace_steps(trace_dir: Path) -> list[dict]:
    ...
    if conv.is_dir():
        ...
        if steps:
            steps = _append_extra_screenshots(steps, trace_dir)
    if not steps:
        hist = trace_dir / "history.txt"
        if hist.is_file():
            steps = _parse_history_txt(hist.read_text(), click_targets, trace_dir)
            steps = _append_extra_screenshots(steps, trace_dir)
    return _attach_missing_screenshots(steps, trace_dir, trace_dir.name)
...
def _step_screenshot_url(trace_dir: Path, step_num: int, trace_name: str) -> str | None:
    """Prefer packaged public assets (Vercel), then local results traces."""
    public_shots = ROOT / "public" / "bakeoff-traces" / trace_name / "screenshots"
    local_shots = trace_dir / "screenshots"
    for shots, prefix in (
        (public_shots, f"/bakeoff-traces/{trace_name}/screenshots"),
        (local_shots, f"/api/bakeoff/traces/{trace_name}/screenshots"),
    ):
        if (shots / f"bbox_{step_num}.png").is_file():
            return f"{prefix}/bbox_{step_num}.png"
        if (shots / f"step_{step_num}.png").is_file():
            return f"{prefix}/step_{step_num}.png"
    return None


def _attach_missing_screenshots(steps: list[dict], trace_dir: Path, trace_name: str) -> list[dict]:
    """If conversation steps lack screenshot_url, fill from screenshot files on disk."""
    if any(s.get("screenshot_url") for s in steps):
        return steps
    shots_dir = trace_dir / "screenshots"
    public_dir = ROOT / "public" / "bakeoff-traces" / trace_name / "screenshots"
    src = shots_dir if shots_dir.is_dir() else public_dir
    if not src.is_dir():
        return steps
    nums = sorted(
        {
            ...
            for m in [re.search(r"(?:bbox|step)_(\d+)\.png$", p.name)]
            if m
        }
    )
    if not nums:
        ...
        return [
            {
                ...
                "thought_detail": {},
                "outcome": "neutral",
                "screenshot_url": _step_screenshot_url(trace_dir, n, trace_name),
            }
            for n in nums
            if _step_screenshot_url(trace_dir, n, trace_name)
        ]
    for i, step in enumerate(steps):
        n = step.get("step") or (i + 1)
        if not step.get("screenshot_url"):
            step["screenshot_url"] = _step_screenshot_url(trace_dir, int(n), trace_name)
    return steps
...
def _without_vapi(comparative: dict) -> dict:
    """Drop Vapi from comparative payloads; remap winners to Bland vs Retell."""
    reviews = []
    for rev in comparative.get("reviews") or []:
        rev = dict(rev)
        per = dict(rev.get("per_platform") or {})
        per.pop("vapi", None)
        rev["per_platform"] = per
        runs = dict(rev.get("runs") or {})
        runs.pop("vapi", None)
        rev["runs"] = runs

        winner = rev.get("most_likely_to_use")
        runner = rev.get("runner_up")
        if winner not in ACTIVE_PLATFORMS:
            if runner in ACTIVE_PLATFORMS:
                winner, runner = runner, None
            else:
                winner = _pick_bland_or_retell(runs, per)
                runner = "retell" if winner == "bland" else "bland" if winner else None
            rev["most_likely_to_use"] = winner
            rev["runner_up"] = runner
            if winner and (rev.get("why_winner") or "").lower().find("vapi") >= 0:
                rev["why_winner"] = (
                    f"Among Bland and Retell, persona pick is {PLATFORM_LABEL.get(winner, winner)}."
                )
        elif runner == "vapi":
            other = "retell" if winner == "bland" else "bland"
            rev["runner_up"] = other if other in runs or other in per else None
...
def _pick_bland_or_retell(runs: dict, per: dict) -> str | None:
    scores: dict[str, tuple] = {}
    for plat in ("bland", "retell"):
        r = runs.get(plat) or {}
        p = per.get(plat) or {}
        ok = 1 if r.get("success") or p.get("would_complete_again") else 0
        ease = {"easy": 2, "medium": 1, "hard": 0}.get(str(p.get("ease") or "").lower(), 0)
        scores[plat] = (ok, ease)
