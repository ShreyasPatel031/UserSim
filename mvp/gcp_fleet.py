...
def _ssh_wait_first_frame(
    *,
    ...
    """Poll seed for live/{agent}/step_0.png via short IAP checks, SCP, publish.

    Brief SSH probes (not one long-held session) so IAP contention cannot stall
    past fleet completion. publish_live_frame patches in-memory study immediately.
    """
    ...
    def _ssh(cmd: str, *, timeout: float = 20.0) -> subprocess.CompletedProcess[str]:
        b64 = b64mod.b64encode(cmd.encode()).decode()
        ssh_cmd = f"bash -c 'echo {b64} | base64 -d | bash'"
    ...
    probe = f"""
set -euo pipefail
LIVE="/home/shreyaspatel/usersim/live/{agent_id}"
[[ -d "$HOME/usersim/live/{agent_id}" ]] && LIVE="$HOME/usersim/live/{agent_id}"
if [[ -f "$LIVE/step_0.png" ]] && [[ -f "$LIVE/step_0.ready" ]] \\
  && [[ $(wc -c < "$LIVE/step_0.png") -gt 100 ]] \\
  && grep -q '{study_id}' "$LIVE/step_0.ready"; then
  echo "$LIVE/step_0.png"
  exit 0
fi
exit 2
"""
    ...
    while time.time() < deadline:
        try:
            proc = _ssh(probe, timeout=25.0)
        except Exception as exc:  # noqa: BLE001
            print(f"ssh live-frame probe error: {exc}", flush=True)
            time.sleep(0.4)
            continue
        if proc.returncode == 0:
            for ln in reversed((proc.stdout or "").splitlines()):
                ln = ln.strip()
                if ln.endswith("step_0.png"):
                    remote_png = ln
                    found = True
                    break
            if found:
        ...
        time.sleep(0.25)
    if not found:
        print(f"ssh live-frame probe timeout ({timeout_s}s) on {name}", flush=True)
        return None
    ...
    with tempfile.TemporaryDirectory(prefix="usersim-live-") as td:
        ...
        # Optional companion metadata (boxes / title) written by fast_first_frame.
        meta: dict[str, Any] = {}
        remote_json = remote_png.rsplit(".", 1)[0] + ".json"
        ...
        try:
            ...
            if scp_json.returncode == 0 and local_json.is_file():
                meta = json.loads(local_json.read_text())
    ...
    *,
    ...
    if status == "RUNNING" and code_uri and job_uri:
        ...
        try:
            ...
            print(f"hot seed dispatch failed ({exc}); falling back to reset", flush=True)
    ...
    if status in {"TERMINATED", "STOPPED"}:
        ...
        # Last resort: reboot so startup-script re-runs.
        op = client.reset(project=project, zone=zone, instance=name)
        op.result(timeout=300)
    else:
        time.sleep(5)
        op = client.start(project=project, zone=zone, instance=name)
...
async def run_study_on_gcp_fleet(
    *,
    ...
    # List seeds in parallel with packing so warm path isn't blocked on upload.
    seed_list_task = (
        asyncio.create_task(asyncio.to_thread(_list_seed_instances, project))
        if USE_WARM_SEEDS
        else None
    )
    ...
    # Overflow shards become Spot copies from the image.
    seed_pool: list[tuple[Any, str]] = []
    if seed_list_task is not None:
        seed_pool = await seed_list_task
    ...
    if n_seed:
        await _status(
            f"Using {n_seed} warm seed(s) first"
            + (f", then {n_spot} Spot copies" if n_spot else " (no Spot copies needed)")
        )
    image_link = ""
    if n_spot:
        try:
    ...
    async def _launch_shard(meta: dict[str, Any]) -> tuple[str, str, str]:
        ...
        if idx in seed_assignments:
            ...
            meta["zone"] = zone
            meta["kind"] = "seed"
            await _status(f"Shard {idx} hot on warm seed {name} ({zone})")
