...
- study: 1f00daaf-b8b3-43fe-9f6a-e5afa7eded07
- product: https://excalidraw.com
- pass: false
- failure file: /workspace/results/tonight/excalidraw-20260926T034744Z/failures.json
...
| Page opened on the assigned site (`page_opened`) | missing field page_open_at_ts; missing field ax_tree | <= 5s from agent creation, URL host matches the assigned site, accessibility tree present | FAIL |
| Study completed with a summary (`study_complete`) | running aborted | status=complete, summary present, no harness abort | FAIL |
| Study budget (`study_budget`) | 22.8s | <= 480s for the whole study (observed max 408s) | PASS |
...
| Per-phase milliseconds (`phase_ms`) | missing field phase_ms | each agent has phase_ms.session_ready, page_open, first_action, final_screenshot | FAIL |
...
| Final URL and page text for the judge (`judge_inputs`) | missing field final_url; missing field state_sig.text | final_url plus state_sig.text or ax_tree on every agent | FAIL |
...
| Every task has a completion rate (`task_completion_rates`) | missing no by_task rows; missing task Draw a simple rectangle on the canvas; missing task Find how to export or share the drawing | each task has a product ok/n rate | FAIL |
...
- Product (competitor_1): 0/8
- Product (competitor_2): 0/8
...
- page_opened: value=missing field page_open_at_ts; missing field ax_tree threshold=<= 5s from agent creation, URL host matches the assigned site, accessibility tree present (missing field page_open_at_ts; missing field ax_tree: 0/24 agents)
- study_complete: value=running aborted threshold=status=complete, summary present, no harness abort (missing field page_open_at_ts; missing field ax_tree: 0/24 agents)
- time_to_first_action: value=missing field page_open_at_ts threshold=median <= 5s and max <= 10s at 24 agents (missing field page_open_at_ts)
- phase_ms: value=missing field phase_ms threshold=each agent has phase_ms.session_ready, page_open, first_action, final_screenshot (missing_agents=24)
...
- judge_inputs: value=missing field final_url; missing field state_sig.text threshold=final_url plus state_sig.text or ax_tree on every agent
...
- task_completion_rates: value=missing no by_task rows; missing task Draw a simple rectangle on the canvas; missing task Find how to export or share the drawing threshold=each task has a product ok/n rate
