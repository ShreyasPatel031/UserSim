...
- `time_to_first_value`: 3.593s (threshold <= 10s from URL submit until the first click, type, or scroll is visible in the live UI) PASS
- `total_time`: not ready (threshold <= 480s from URL submit until the report is ready) FAIL

- study: 04d5ab41-82dc-455c-b1dc-774b493fcfc4
- product: https://linear.app
- pass: false
- failure file: results/tonight/linear-20260926T055111Z/failures.json
- failed runs: our infrastructure=0, model timeout=0, stuck=0, product=1, no_first_action=0

| Gate | Value | Threshold | Result |
| --- | --- | --- | --- |
| Time to first value (`time_to_first_value`) | 3.593s | <= 10s from URL submit until the first click, type, or scroll is visible in the live UI | PASS |
...
| Study budget (`study_budget`) | 11.8s | <= 480s for the whole study (observed max 408s) | PASS |
| First real action (`first_action`) | not measured | live abort is time_to_first_action | PASS |
| Time to first action (`time_to_first_action`) | median=1.362s max=2.396s n=24/24 | median <= 5s and max <= 10s at 24 agents | PASS |
