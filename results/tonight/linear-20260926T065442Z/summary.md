...
- `time_to_first_value`: 8.005s (threshold <= 10s from URL submit until the first click, type, or scroll is visible in the live UI) PASS
- `total_time`: not ready (threshold <= 480s from URL submit until the report is ready) FAIL

- study: c1832e16-3271-4a4b-b3e9-b6661a54b1e4
- product: https://linear.app
- pass: false
- failure file: results/tonight/linear-20260926T065442Z/failures.json
- failed runs: our infrastructure=0, model timeout=0, stuck=0, product=0, no_first_action=11

| Gate | Value | Threshold | Result |
| --- | --- | --- | --- |
| Time to first value (`time_to_first_value`) | 8.005s | <= 10s from URL submit until the first click, type, or scroll is visible in the live UI | PASS |
| Total time (`total_time`) | not ready | <= 480s from URL submit until the report is ready | FAIL |
| Full 24-agent matrix (`full_matrix`) | 24 | >= 24 (smaller runs are smoke-only) | FAIL |
| Agent count (`agent_count`) | 18 | >= 24 | FAIL |
| Persona count (`persona_count`) | 5 | >= 4 | PASS |
| Task count (`task_count`) | 2 | >= 2 | PASS |
| Site count (`site_count`) | 3 | >= 3 | PASS |
| Page opened on the assigned site (`page_opened`) | 7/18 within 5s | <= 5s from agent creation, URL host matches the assigned site, accessibility tree present | FAIL |
| Study completed with a summary (`study_complete`) | running aborted | status=complete, summary present, no harness abort | FAIL |
| Study budget (`study_budget`) | 13.2s | <= 480s for the whole study (observed max 408s) | PASS |
| First real action (`first_action`) | not measured | live abort is time_to_first_action | PASS |
| Time to first action (`time_to_first_action`) | missing field page_open_at_ts | median <= 5s and max <= 10s at 24 agents | FAIL |
| Per-phase milliseconds (`phase_ms`) | 18/18 | each agent has phase_ms.session_ready, page_open, first_action, final_screenshot | PASS |
...
| Final URL and page text for the judge (`judge_inputs`) | 18/18 | final_url plus state_sig.text or ax_tree on every agent | PASS |
| Failed-step phase and reason (`failed_step`) | missing field failed_step_phase; missing field failed_step_reason | every failed run has failed_step.phase and failed_step.reason | FAIL |
| Product task completion (`product_task_completion`) | 0/7 (0%) | >= 50% of product runs (>= 4/7) | FAIL |
...
| Run issue share (`run_issue_rate`) | 0/18 (0%) | <= 25% of runs | PASS |
...
| First-screen and opening-frame runs stay in the product denominator (`first_screen_not_excluded`) | opening_or_first_screen=0 product_n=7 excluded=0 | excluded=0 (a stuck run is a failure, not a drop) | PASS |

## Competitor task completion

Reported separately. These runs do not count toward the product gate.

- https://asana.com/ (competitor_1): 0/5
- https://trello.com/ (competitor_2): 0/6

## Product task completion

- confirmed: 0/7
- past the first screen (structural): 7
...
- full_matrix: value=24 threshold=>= 24 (smaller runs are smoke-only) (agents=18)
- agent_count: value=18 threshold=>= 24
- page_opened: value=7/18 within 5s threshold=<= 5s from agent creation, URL host matches the assigned site, accessibility tree present
- study_complete: value=running aborted threshold=status=complete, summary present, no harness abort (missing field page_open_at_ts: 11/18 agents have no page-open or session-ready stamp)
- time_to_first_action: value=missing field page_open_at_ts threshold=median <= 5s and max <= 10s at 24 agents (missing field page_open_at_ts: 11/18 agents have no page-open or session-ready stamp)
...
- failed_step: value=missing field failed_step_phase; missing field failed_step_reason threshold=every failed run has failed_step.phase and failed_step.reason
- product_task_completion: value=0/7 (0%) threshold=>= 50% of product runs (>= 4/7) (judge_yes=0 structural_past_first_screen=7 first_screen_or_opening=0 opening_frame=0 stuck=0 bb_losses_in_product=0)
