...
- `time_to_first_value`: 2.328s (threshold <= 10s from URL submit until the first click, type, or scroll is visible in the live UI) PASS
- `total_time`: 104.178s (threshold <= 480s from URL submit until the report is ready) PASS

- study: dfb33c58-7f71-4b1b-8103-ee4ebe1a33cd
- product: https://linear.app
- pass: false
- failure file: results/tonight/linear-20260926T044916Z/failures.json
- failed runs: our infrastructure=0, model timeout=0, stuck=0, product=16, no_first_action=0

| Gate | Value | Threshold | Result |
| --- | --- | --- | --- |
| Time to first value (`time_to_first_value`) | 2.328s | <= 10s from URL submit until the first click, type, or scroll is visible in the live UI | PASS |
| Total time (`total_time`) | 104.178s | <= 480s from URL submit until the report is ready | PASS |
...
| Study budget (`study_budget`) | 104.8s | <= 480s for the whole study (observed max 408s) | PASS |
| First real action (`first_action`) | not measured | live abort is time_to_first_action | PASS |
| Time to first action (`time_to_first_action`) | median=0.0s max=0.0s n=24/24 | median <= 5s and max <= 10s at 24 agents | PASS |
...
| Bland-style report page (`report_page`) | http://127.0.0.1:3000/report?study=dfb33c58-7f71-4b1b-8103-ee4ebe1a33cd | /report?study=<id> shell with overview and trace tabs | PASS |
...
| Product weakness from a run past the first screen (`product_weakness`) | 0/2 | >= 1 weakness with agent, step, AX or URL, and the final screenshot | FAIL |
| Top weakness is not only the homepage stall (`top_weakness_not_homepage_only`) | The new issue form is hard to find because this page has no issue title field. | at least one weakness is a real product issue from a run past the first screen | FAIL |
...
- product_weakness: value=0/2 threshold=>= 1 weakness with agent, step, AX or URL, and the final screenshot
- top_weakness_not_homepage_only: value=The new issue form is hard to find because this page has no issue title field. threshold=at least one weakness is a real product issue from a run past the first screen (real_weaknesses=0)
