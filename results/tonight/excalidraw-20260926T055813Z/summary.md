...
- `time_to_first_value`: 2.494s (threshold <= 10s from URL submit until the first click, type, or scroll is visible in the live UI) PASS
- `total_time`: not ready (threshold <= 480s from URL submit until the report is ready) FAIL

- study: 33cee22a-85fb-466b-b6c4-4ca8ef4b6de7
- product: https://excalidraw.com
- pass: false
- failure file: results/tonight/excalidraw-20260926T055813Z/failures.json
- failed runs: our infrastructure=0, model timeout=0, stuck=0, product=2, no_first_action=0

| Gate | Value | Threshold | Result |
| --- | --- | --- | --- |
| Time to first value (`time_to_first_value`) | 2.494s | <= 10s from URL submit until the first click, type, or scroll is visible in the live UI | PASS |
...
| Page opened on the assigned site (`page_opened`) | 22/24 within 5s | <= 5s from agent creation, URL host matches the assigned site, accessibility tree present | FAIL |
| Study completed with a summary (`study_complete`) | running aborted | status=complete, summary present, no harness abort | FAIL |
| Study budget (`study_budget`) | 12.5s | <= 480s for the whole study (observed max 408s) | PASS |
| First real action (`first_action`) | not measured | live abort is time_to_first_action | PASS |
| Time to first action (`time_to_first_action`) | median=1.483s max=2.031s n=24/24 | median <= 5s and max <= 10s at 24 agents | PASS |
...
- page_opened: value=22/24 within 5s threshold=<= 5s from agent creation, URL host matches the assigned site, accessibility tree present (FAIL page_opened: 22/24 agents opened the assigned site within 5s (opened after 5s=2))
- study_complete: value=running aborted threshold=status=complete, summary present, no harness abort (FAIL page_opened: 22/24 agents opened the assigned site within 5s (opened after 5s=2))
