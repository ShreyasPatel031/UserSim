...
- `time_to_first_value`: 2.713s (threshold <= 10s from URL submit until the first click, type, or scroll is visible in the live UI) PASS
- `total_time`: 39.279s (threshold <= 480s from URL submit until the report is ready) PASS

- study: 09472fc2-38f1-4ee4-b966-0fa0b4be819e
- product: https://excalidraw.com
- pass: true
- failure file: results/tonight/excalidraw-20260926T061826Z/failures.json
- failed runs: our infrastructure=0, model timeout=0, stuck=0, product=9, no_first_action=0

| Gate | Value | Threshold | Result |
| --- | --- | --- | --- |
| Time to first value (`time_to_first_value`) | 2.713s | <= 10s from URL submit until the first click, type, or scroll is visible in the live UI | PASS |
| Total time (`total_time`) | 39.279s | <= 480s from URL submit until the report is ready | PASS |
...
| Study budget (`study_budget`) | 39.8s | <= 480s for the whole study (observed max 408s) | PASS |
| First real action (`first_action`) | not measured | live abort is time_to_first_action | PASS |
| Time to first action (`time_to_first_action`) | median=1.177s max=1.26s n=24/24 | median <= 5s and max <= 10s at 24 agents | PASS |
...
| Failed-step phase and reason (`failed_step`) | 9 failed runs | every failed run has failed_step.phase and failed_step.reason | PASS |
| Product task completion (`product_task_completion`) | 8/8 (100%) | >= 50% of product runs (>= 4/8) | PASS |
| Bland-style report page (`report_page`) | http://127.0.0.1:3000/report?study=09472fc2-38f1-4ee4-b966-0fa0b4be819e | /report?study=<id> shell with overview and trace tabs | PASS |
...
- https://www.tldraw.com/ (competitor_1): 7/8
