...
- `time_to_first_value`: 8.887s (threshold <= 10s from URL submit until the first click, type, or scroll is visible in the live UI) PASS
- `total_time`: not ready (threshold <= 480s from URL submit until the report is ready) FAIL

- study: c022e582-3826-4c8e-a2ce-f11271e23be7
- product: https://linear.app
- pass: false
- failure file: /workspace/results/tonight/linear-20260926T034437Z/failures.json
- failed runs: our infrastructure=0, model timeout=0, stuck=2, product=0, no_first_action=0

| Gate | Value | Threshold | Result |
| --- | --- | --- | --- |
| Time to first value (`time_to_first_value`) | 8.887s | <= 10s from URL submit until the first click, type, or scroll is visible in the live UI | PASS |
...
| Study budget (`study_budget`) | 67.7s | <= 480s for the whole study (observed max 408s) | PASS |
| First real action (`first_action`) | not measured | live abort is time_to_first_action | PASS |
| Time to first action (`time_to_first_action`) | median=5.503s max=6.472s n=24/24 | median <= 5s and max <= 10s at 24 agents | FAIL |
| Per-phase milliseconds (`phase_ms`) | missing field phase_ms.final_screenshot | each agent has phase_ms.session_ready, page_open, first_action, final_screenshot | FAIL |
...
| First-screen and opening-frame runs stay in the product denominator (`first_screen_not_excluded`) | opening_or_first_screen=6 product_n=8 excluded=0 | excluded=0 (a stuck run is a failure, not a drop) | PASS |
...
- confirmed: 0/8
- past the first screen (structural): 2
- first screen or opening frame: 6

## Failed gates

- total_time: value=not ready threshold=<= 480s from URL submit until the report is ready (8-minute study budget.)
- study_complete: value=running aborted threshold=status=complete, summary present, no harness abort (FAIL stuck: 2 agent(s) repeated the same action 3 times with no URL/DOM change. t1__p1__product phase=acting error= last_action=click New issue; t1__p2__product phase=acting error= last_action=click New issue)
- time_to_first_action: value=median=5.503s max=6.472s n=24/24 threshold=median <= 5s and max <= 10s at 24 agents
- phase_ms: value=missing field phase_ms.final_screenshot threshold=each agent has phase_ms.session_ready, page_open, first_action, final_screenshot (missing_agents=24)
...
- product_task_completion: value=0/8 (0%) threshold=>= 50% of product runs (>= 4/8) (judge_yes=0 structural_past_first_screen=2 first_screen_or_opening=6 opening_frame=0 stuck=2 bb_losses_in_product=0)
