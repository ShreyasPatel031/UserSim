...
- `time_to_first_value`: 2.104s (threshold <= 10s from URL submit until the first click, type, or scroll is visible in the live UI) PASS
- `total_time`: 30.74s (threshold <= 480s from URL submit until the report is ready) PASS

- study: 40e719b1-5e63-46af-9ca2-3f322868b513
- product: https://excalidraw.com
- pass: true
- failure file: results/tonight/excalidraw-20260926T062635Z/failures.json
- failed runs: our infrastructure=0, model timeout=0, stuck=0, product=9, no_first_action=0

| Gate | Value | Threshold | Result |
| --- | --- | --- | --- |
| Time to first value (`time_to_first_value`) | 2.104s | <= 10s from URL submit until the first click, type, or scroll is visible in the live UI | PASS |
| Total time (`total_time`) | 30.74s | <= 480s from URL submit until the report is ready | PASS |
...
| Study budget (`study_budget`) | 31.2s | <= 480s for the whole study (observed max 408s) | PASS |
| First real action (`first_action`) | not measured | live abort is time_to_first_action | PASS |
| Time to first action (`time_to_first_action`) | median=1.446s max=1.711s n=24/24 | median <= 5s and max <= 10s at 24 agents | PASS |
...
| Bland-style report page (`report_page`) | http://127.0.0.1:3000/report?study=40e719b1-5e63-46af-9ca2-3f322868b513 | /report?study=<id> shell with overview and trace tabs | PASS |
