...
- `time_to_first_value`: 1.28s (threshold <= 10s from URL submit until the first click, type, or scroll is visible in the live UI) PASS
- `total_time`: 35.172s (threshold <= 480s from URL submit until the report is ready) PASS

- study: 38deb1cc-8bf3-4d6a-a5b4-ca147c8cf160
- product: https://excalidraw.com
- pass: true
- failure file: /tmp/usersim_e2e2_excalidraw24d/failures.json
- failed runs: our infrastructure=0, model timeout=0, stuck=0, product=9, no_first_action=0

| Gate | Value | Threshold | Result |
| --- | --- | --- | --- |
| Time to first value (`time_to_first_value`) | 1.28s | <= 10s from URL submit until the first click, type, or scroll is visible in the live UI | PASS |
| Total time (`total_time`) | 35.172s | <= 480s from URL submit until the report is ready | PASS |
...
| Study budget (`study_budget`) | 35.6s | <= 480s for the whole study (observed max 408s) | PASS |
...
| Bland-style report page (`report_page`) | http://127.0.0.1:3000/report?study=38deb1cc-8bf3-4d6a-a5b4-ca147c8cf160 | /report?study=<id> shell with overview and trace tabs | PASS |
