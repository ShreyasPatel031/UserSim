...
def main() -> int:
    ...
    def _on_done(row: dict) -> None:
        ...
        writer.request_save()
        # Trace upload is best-effort; do not block the worker pool on GCS.
        try:
    ...
    if workers == 1:
        ...
        # Hard per-task deadline so one hung Chromium cannot freeze the shard forever.
        # asyncio.wait_for inside the runner is the primary kill; this is the backstop.
        wall_s = task_wall_timeout_s(args.max_actions)
        grace_s = 60.0  # allow runner cleanup after TimeoutError
        ...
        try:
            ...
            # Do not wait for zombie threads stuck in Chromium/CDP — shard must finish.
